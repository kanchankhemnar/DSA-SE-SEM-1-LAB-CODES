    ...
    else
    {
        ...
        while(!read_file.eof())  //search till end of file
        {
            if(stud_rec.roll_no == key)  //if key is equal to correct roll number
            {
                ...
                flag = true;  //set flag
                // cout<<"\nRoll No.\tName\t\tDoB\t\tPercentage\tAddress\n---------------------------------------------------------------------------";
                cout<<"\n"<<"Roll no "<<stud_rec.roll_no<<"\t\tName"<<stud_rec.stud_name<<"\nDOB: "<<stud_rec.stud_DoB.day<<"/"<<stud_rec.stud_DoB.month<<"/"<<stud_rec.stud_DoB.year<<"\t\tPercentage "<<stud_rec.stud_percent<<"\nAddress"<<stud_rec.stud_address;  //display student details
                break;
            }
            else
            {
